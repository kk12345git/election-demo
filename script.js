...
function toggleAudio() {
  ...
  if (isMuted) {
    ...
    // If unmuting, potentially restart audio for current scheme
    showScheme(currentSchemeId);
  }
}
...
function transitionToScheme(id) {
  ...
  // Interstitial Stage (Loading screen)
...
function initAutoCycle() {
  if (autoTimer) clearTimeout(autoTimer);
  // Fail-safe timeout: 180s (3 minutes)
...
function setLang(lang) {
  ...
  if (scLbl) scLbl.textContent = TX[lang]['sc-lbl'];
  
  const fcopy = document.getElementById('fcopy');
  if (fcopy) fcopy.textContent = TX[lang]['fcopy'];

  // Language button state
  document.querySelectorAll('.lb').forEach(b => {
      const isEnglishBtn = b.textContent.trim() === 'EN';
      b.classList.toggle('act', (lang === 'en' && isEnglishBtn) || (lang === 'ta' && !isEnglishBtn));
  });
...
window.addEventListener('load', () => {
  // Clear any existing synth activity
  window.speechSynthesis.cancel();
...
// Event Listeners for Audio Priming
